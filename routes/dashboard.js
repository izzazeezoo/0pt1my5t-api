...
router.post("/dashboard/project_manager/create", authorization, (req, res) => {
    ...
    if (!project_name || !contract_num || !contract_value || !status || !team_name || !Array.isArray(team_members)) {
                ...
                (err, projectResult) => {
                    ...
                    db.query(
                        ...
                        (err, teamResult) => {
                            if (err) {
                                console.error(err);
                                return res.status(500).send({ message: "Error inserting team." });
                            }

                            const teamId = teamResult.insertId;

                            // Insert team members
                            const teamMemberValues = team_members.map(member => [teamId, member.user_id, member.role]);
                            ...
                            db.query(
                                ...
                                (err) => {
                                    if (err) {
                                        console.error(err);
                                        return res.status(500).send({ message: "Error inserting team members." });
                                    }
