...
router.post("/team/find", authorizePM, (req, res) => {
    ...
    LIMIT ?
        ...
        (err, results) => {
            ...
            if (results.length === 0) {
                return res.status(404).send({ message: "No profiles found matching the role." });
            }
...
router.post("/team/assign", authorizePM, (req, res) => {
    ...
    db.query(
        ...
        (err) => {
            ...
            return res.status(201).send({ message: "Members assigned successfully. Current Status: Waiting for Approval." });
        }
    );
});
