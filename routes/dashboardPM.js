...
const { verifyUserGID, verifyPMRole, verifyPrimaryPM } = require("../middleware/verification");
...
router.put(
    ...
    async (req, res) => {
        ...
        if (
            !project_name || !project_description || !contract_num ||
            !contract_value || !status || !size || !project_id
        ) {
        ...
        try {
            ...
            await queryAsync(`DELETE FROM contracts WHERE project_id = ?`, [project_id]);
...
router.post("/team/assign", authorizePM, async (req, res) => {
	...
	if (!team_id || !members || !Array.isArray(members)) {
		return res.status(400).send({ message: "Team ID and members are required." });
	}
	...
	try {
		...
		return res.status(201).send({
			message: "Members assigned successfully. Current Status: Waiting for Approval.",
...
router.post("/tribe/find/:tribeId", authorizePM, verifyUserGID, (req, res) => {
    ...
    if (!job_group || !experience_level || !required_count || !tribe_id) {
        return res.status(400).send({ 
            message: "Job group, experience level, required count, and team ID are required." 
        });
    }
...
router.post("/tribe/assign", authorizePM, async (req, res) => {
    try {
        ...
        for (const member of members) {
            ...
            if (!member.user_id || !member.role || !member.job_group || !member.rank) {
            ...
            if (rankName !== 'ANGGOTA' && member.job_group.toUpperCase() !== 'SENIOR MANAGEMENT') {
                errors.push(`User ${member.user_id} must be Senior Management for rank ${member.rank}`);
                continue;
            }
        ...
        return res.status(201).json({
            message: "Tribe members assigned successfully. Current Status: Waiting for Approval.",
